...
##############~~~~~~~~~~~~~~~~~~~~~~Do distance calculations Puerto Rico with shape file~~~~~~~~~~~~~~~~#####################
##########################################
...
# #that occurred between 1970 and 2017. The previous datset 
# #included hurricanes as far back as 1851 which resulted
# #in the vector of names being too large to place in one
...
########################################This worked olay but Some locations were not identified properly#############################
########################################It is a good security measure though if you want to name your locations######################
###################################################################quickly###########################################################
...
intp_1<-lapply(intp_1, function(x) {as.data.frame(x);x})
...
#add names to each of the list items
...
interpol_hur$time<-round(interpol_hur$time, 0)
...
new_cropset_at_full<-merge(new_cropset_at, Cen90_crop, by = "ID", all=T)
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Second attempt at naming the locations<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
#
#Use name assigning functoin to assign name to venezuela then apply the codes below to take away those places that were 
#incorrectly assigned to venezuela due to the wide marine borders. 
#
#Must Assign Venezuala and Columbia before Aruba since the shape file will absorb Aruba due to proximity.
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Get names<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
# cropset_at_full[(which(cropset_at_full$Long>=-(venbox[1,2]) & cropset_at_full$Long<=-(venbox[1,1]) & cropset_at_full$Lat.y>=(venbox[2,1]) & cropset_at_full$Lat.y<=(venbox[2,2]))),16]<-"Venezuela"
...
gilbert_track<-subset(jamaica, jamaica$name=="GILBERT", drop= TRUE)
...
fix_distance[4:8]<-NULL
fix_distance[9:12]<-NULL
...
#########################Looking at all the hurricanes from jamaica and Puerto Rico       ######################
########################Night not use this methdology because the storm collection        ######################
########################is more efficient from the new way that I am doing it. All in     ######################
########################all the below does not have to be used but I'll keep it for record######################
...
# fix1<-na.omit(fix_distance)
# 
# #Converting wind speed in hurricane to knots
# fix1$`wspeed at H`<-convert_wind_speed(fix1$`wspeed at H`, old_metric = "kmph", new_metric = "knots", round = NULL)
# 
...
#Need to fix the interpolation of the hurricanes. This can be done by using the tint function in creat full track. 
